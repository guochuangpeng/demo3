#include "relative_localization3D.h"
...
uwb_relative_localization::uwb_relative_localization()
{
   ...
   /******相对定位算法参数******/
   private_nh_.param<double>("para_Gamma", para_Gamma, 10.0);//para_Gamma = para_alpha * 单位矩阵I作为估计器增益矩阵 
   /******激光测高参数******/
   ...
   //para_Sigma = Eigen::MatrixXd::Identity(3,3);//单位矩阵
   para_pho0 << 1, 0;//估计器参数初始值
   //para_Gamma = para_alpha;//估计器增益
...
void uwb_relative_localization::height_callback(const sensor_msgs::Range::ConstPtr& msg)
{
   if(init_lidar){   
      ...
      high_sensor_value_old = high_sensor_value;
      init_lidar = false;
   }else{
      //high_sensor_value_old = high_sensor_value;
      high_sensor_value = msg->range;//单位：m 激光测距参数，未矫正距离
      ...
      pos.z_pos = uav_height;//uav底部到地面的距离
      pos.lidar_value = high_sensor_value;//单位：m 激光测距参数，未矫正距离
      //flow_Phi(2,0) = high_sensor_value - high_sensor_value_old; 
   }
   /*
...
void uwb_relative_localization::distance_callback(const uwb::uwbrangemsg::ConstPtr& msg)
{
   if(is_first_step) {      
      ...
      para_pho = para_pho0;
      para_sigma(0,0) = r1 * para_pho(0,0);
      para_sigma(1,0) = r1 * para_pho(1,0);
      para_sigma(2,0) = r2 * ((4*para_pho(0,0)*para_pho(0,0)*para_pho(0,0))-(3*para_pho(0,0)));
      q_esti = q_esti0;
      is_first_step = false;
   }
   else{
...
{
   flow_Phi(2,0) = high_sensor_value - high_sensor_value_old; 
   flow_Phi_2_norm_2 = (flow_Phi(0,0)*flow_Phi(0,0)) + (flow_Phi(1,0)*flow_Phi(1,0)) + (flow_Phi(2,0)*flow_Phi(2,0));
   //flow_Phi_2_norm_2 = flow_Phi.squaredNorm();
   para_zeta = (0.5) * ( (r_now*r_now) - (r_old*r_old) - flow_Phi_2_norm_2 );
   para_epsilon = para_zeta - ( flow_Phi.transpose() * q_esti );
   q_esti = q_esti + flow_Phi + ( para_Gamma * flow_Phi * para_epsilon );//ENU
   q_esti_2_norm = sqrt( (q_esti(0,0)*q_esti(0,0)) + (q_esti(1,0)*q_esti(1,0)) + (q_esti(2,0)*q_esti(2,0)));
   ...
   pos.z_pos_relative = q_esti(2,0);
   pos.sigma1 = para_sigma(0,0);
   pos.sigma2 = para_sigma(1,0);
   pos.sigma3 = para_sigma(2,0);
   //ROS_INFO("high_sensor_value_out:%f",pos.lidar_value);
   //ROS_INFO("uav_height_out:%f",pos.z_pos);
   ...
   ROS_INFO("Position: x_pos_relative:%f y_pos_relative:%f z_pos_relative:%f", pos.x_pos_relative, pos.y_pos_relative, pos.z_pos_relative);
}

int main(int argc, char **argv)
{
  ros::init(argc, argv, "relative_localization3D");
  uwb_relative_localization rel_pos3D;
  ros::spin();
