...
{   
  ...
  private_nh_.param<double>("para_beta", para_beta, 5.0);//速度控制器参数
  private_nh_.param<double>("para_alpha", para_alpha, 1.0);//速度控制器参数
...
void rel_controller::uwb_rel_pos_cb(const relative_localization::relative_position::ConstPtr& msg)
{
    ...
    if(uwb_phase){ 
      u_relative = ( (-1.0) * para_beta * q_rel_uwb ) + ( para_alpha * distance * para_sigma );//速度控制器
      u_relative_2_norm = sqrt( (u_relative(0,0)*u_relative(0,0)) + (u_relative(1,0)*u_relative(1,0)) + (u_relative(2,0)*u_relative(2,0)));
      //u_relative_2_norm = u_relative.norm();
...
{
  ros::init(argc, argv, "uav_movement3D");
  rel_controller position3D;
  ros::spin();
  /*ros::Rate r(10); 
  while(ros::ok()){
     // read
